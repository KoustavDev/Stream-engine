...
async function init() {
  try {
    ...
    console.log(`Starting 720p transcoding process for ${filename}`);
    ...
    const originalVideoPath = "/tmp/original-vid-720p.mp4";
    ...
    const outputDir = `/tmp/output-720p/${basePath}`;
    fs.mkdirSync(`${outputDir}/stream_1`, { recursive: true });

    // Use FFmpeg to create HLS segments for 720p only
    const ffmpegCommand = `ffmpeg -y -i "${filePath}" \
-map 0:v -filter:v:0 scale=1280:720 \
-c:v:0 libx264 -b:v:0 2800k -maxrate:v:0 2996k -bufsize:v:0 4200k \
-map 0:a -c:a aac -b:a:0 128k -ac 2 \
-f hls \
-hls_time 10 \
-hls_playlist_type vod \
-hls_flags independent_segments \
-hls_segment_filename "${outputDir}/stream_1/segment_%03d.ts" \
${outputDir}/stream_1/index.m3u8`;

    console.log(
      "Executing FFmpeg command to create 720p HLS segments locally..."
    );
    ...
    console.log("720p FFmpeg transcoding completed successfully");

    // Upload the directory structure to S3
    console.log("Uploading 720p HLS content to S3...");
    await uploadDirectoryToS3(`${outputDir}/stream_1`, `${basePath}/stream_1`);

    console.log("720p HLS segments and playlist have been uploaded to S3");
    console.log(
      `720p content available at: s3://${productionBucket}/${basePath}/stream_1/index.m3u8`
    );
    ...
    console.error("Error during 720p transcoding:", error);
    process.exit(1);
  }
}
