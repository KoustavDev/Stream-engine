...
async function init() {
  try {
    ...
    console.log(`Starting 1080p transcoding process for ${filename}`);
    ...
    const originalVideoPath = "/tmp/original-vid-1080p.mp4";
    ...
    const outputDir = `/tmp/output-1080p/${basePath}`;
    fs.mkdirSync(`${outputDir}/stream_0`, { recursive: true });

    // Use FFmpeg to create HLS segments for 1080p only
...
-hls_segment_filename "${outputDir}/stream_0/segment_%03d.ts" \
${outputDir}/stream_0/index.m3u8`;

    console.log(
      "Executing FFmpeg command to create 1080p HLS segments locally..."
    );
    ...
    console.log("1080p FFmpeg transcoding completed successfully");

    // Upload the directory structure to S3
    console.log("Uploading 1080p HLS content to S3...");
    await uploadDirectoryToS3(`${outputDir}/stream_0`, `${basePath}/stream_0`);

    console.log("1080p HLS segments and playlist have been uploaded to S3");
    console.log(
      `1080p content available at: s3://${productionBucket}/${basePath}/stream_0/index.m3u8`
    );
    ...
    console.error("Error during 1080p transcoding:", error);
    process.exit(1);
  }
}
